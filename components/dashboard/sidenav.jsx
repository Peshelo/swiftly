...
<Command  className="bg-transparent">
  ...
  <CommandList className="text-white">
    <CommandGroup heading="General">
      <CommandItem><Link href="/super_admin"  className={`flex flex-row w-full items-center max-sm:justify-center gap-x-2 py-2 text-white ${currentPath === '/super_admin' ? 'bg-green-600' : 'hover:bg-green-600'}`}><HiViewGrid size={20} />
       <p className="max-sm:hidden">Dashboard</p></Link></CommandItem>
    </CommandGroup>
    <CommandGroup heading="Merchants">
      <CommandItem ><Link href="/super_admin/merchants" className={`flex flex-row w-full items-center max-sm:justify-center gap-x-2 py-2 text-white ${currentPath === '/super_admin/merchants' ? 'bg-green-600' : 'hover:bg-green-600'}`}><HiUserGroup size={20} />
